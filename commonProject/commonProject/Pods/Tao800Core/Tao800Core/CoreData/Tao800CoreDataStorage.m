//
//  Tao800CoreDataStorage.m
//  Tao800Core
//  引用xmpp coredata的创建方式
...
@implementation Tao800CoreDataStorage
...
// It may be invoked on any thread/queue.
    ...
    dispatch_block_t block = ^{
        @autoreleasepool {
            ...
            TBDPRINT(@"%@: Creating managedObjectModel (%@)", [self class], momName);
...
// If path is nil, then NSURL or NSManagedObjectModel will throw an exception
                ...
                TBDPRINT(@"%@: Couldn't find managedObjectModel file - %@", [self class], momName);
            }
...
// It may be invoked on any thread/queue.
    ...
    dispatch_block_t block = ^{
        @autoreleasepool {
            ...
            TBDPRINT(@"%@: Creating persistentStoreCoordinator", [self class]);
...
// If storePath is nil, then NSURL will throw an exception
                    ...
                    TBDPRINT(@"%@: Error creating persistentStoreCoordinator - Nil persistentStoreDirectory",
                            [self class]);
                }
            }
            else {
...
//
    ...
    NSPersistentStoreCoordinator *coordinator = [self persistentStoreCoordinator];
    if (coordinator) {
        TBDPRINT(@"%@: Creating managedObjectContext", [self class]);
...
//
    ...
    NSPersistentStoreCoordinator *coordinator = [self persistentStoreCoordinator];
    if (coordinator) {
        TBDPRINT(@"%@: Creating mainThreadManagedObjectContext", [self class]);
        if ([NSManagedObjectContext instancesRespondToSelector:@selector(initWithConcurrencyType:)])
...
//        TBDPRINT(@"%@: %@ - Merging changes into mainThreadManagedObjectContext", THIS_FILE, THIS_METHOD);
...
// called from maybeSave below, which already does this check.
    for (
            ...
            in willSaveManagedObjectContextBlocks) {
        block();
    }
    ...
    if ([[self managedObjectContext] save:&error]) {
        ...
        for (
                ...
                in didSaveManagedObjectContextBlocks) {
            block();
        }
        [didSaveManagedObjectContextBlocks removeAllObjects];
    }
    else {
        TBDPRINT(@"%@: Error saving - %@ %@", [self class], error, [error userInfo]);
...
- (void)maybeSave:(int32_t)currentPendingRequests {
    ...
    if ([[self managedObjectContext] hasChanges]) {
        if (currentPendingRequests == 0) {
            TBDPRINT(@"%@: Triggering save (pendingRequests=%i)", [self class], currentPendingRequests);
            [self save];
        }
        else {
            NSUInteger unsavedCount = [self numberOfUnsavedChanges];
            if (unsavedCount >= saveThreshold) {
                TBDPRINT(@"%@: Triggering save (unsavedCount=%lu)", [self class], (unsigned long) unsavedCount);
                [self save];
            }
        }
    }
