...
@interface Tao800BannerView ()
- (void)fireTimer:(NSTimer *)theTimer;

@property (nonatomic, assign) BOOL dragging;
@end

@implementation Tao800BannerView {
...
- (void)fireTimer:(NSTimer *)theTimer {
    NSUInteger count = [self.bannerArray count];
    NSInteger tMax = count - 1;
    if (self.currentPage > tMax) {
        self.currentPage = 0;
    }
    NSInteger inx = self.currentPage;
    if (inx < tMax) {
        inx++;
    } else {
        inx = 0;
