...
- (id)initWithName:(NSString *)name {
    ...
    if (self) {
        ...
        _imageSortedList = [[TBSafeMutableArray alloc] init];
        _imageCache = [[TBSafeMutableDictionary alloc] init];
...
+ (NSString *)doubleImageURLPath:(NSString *)urlPath {
    ...
    if (TBIsIPhone6Plus()) {
        urlPath = [urlPathWithNoExtension stringByAppendingString:@"@3x"];
    } else {
...
- (id)imageForURL:(NSString *)URL {
    ...
    if (!image && URL.length>0) {
        ...
        NSRange range1 = [str rangeOfString:@"@2x"];
        NSRange range2 = [str rangeOfString:@"@3x"];
        if (range1.length>0 || range2.length>0) {
            image = [self imageForURL:str fromDisk:YES];
        }
    }
    return image;
}
