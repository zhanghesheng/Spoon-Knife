...
- (void)startCustomLoading {
    ...
    [self.customView.layer addAnimation:shake forKey:@"shakeAnimation"];
}
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeDeterminate) {
        UIActivityIndicatorView *indicatorView = (UIActivityIndicatorView*) indicator;
        ...
        if (self.tag == TBProgressLoadingTag) {
...
- (void) applicationBecomeActive {
    if (mode == MBProgressHUDModeCustomView && self.customView != nil) {
        if (self.tag == TBProgressLoadingTag) {
             _isAnimating = NO;
            [self startCustomLoading];
        }
...
- (void)setTransformForCurrentOrientation:(BOOL)animated {
    ...
    BOOL after7 = RequireSysVerGreaterOrEqual(@"8");
    ...
    if (!TBSystemVersionGreaterThanOrEqualTo(NSFoundationVersionNumber_iOS_8_0)) {
