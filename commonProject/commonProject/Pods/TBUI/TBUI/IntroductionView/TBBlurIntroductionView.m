...
#import "TBBlurIntroductionView.h"

@implementation TBBlurIntroductionView
@synthesize delegate;
...
-(void)initializeViewComponents{
    ...
    if ([TBIntroductionPanel runningiOS7]) {
...
-(void)buildIntroductionWithPanels:(NSArray *)panels{
    Panels = panels;
    for (TBIntroductionPanel *panel in Panels) {
...
-(void)blurTintColorSet:(UIColor*) color{
    if ([TBIntroductionPanel runningiOS7]) {
        self.BlurView.blurTintColor = color;
    }else{
...
-(void)addPanelsToScrollView{
    if (Panels) {
        if (Panels.count > 0) {
            ...
            if (self.LanguageDirection == TBLanguageDirectionLeftToRight) {
...
-(void)buildScrollViewLeftToRight{
    ...
    for (TBIntroductionPanel *panelView in Panels) {
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    if (self.LanguageDirection == TBLanguageDirectionLeftToRight) {
        ...
        if (self.CurrentPanelIndex == (Panels.count)) {
            ...
            if ([(id)delegate respondsToSelector:@selector(introduction:didFinishWithType:)]) {
                [delegate introduction:self didFinishWithType:TBFinishTypeSwipeOut];
            }
        }
        else {
    ...
    else if(self.LanguageDirection == TBLanguageDirectionRightToLeft){
...
-(void)skipIntroduction{
    if ([(id)delegate respondsToSelector:@selector(introduction:didFinishWithType:)]) {
        [delegate introduction:self didFinishWithType:TBFinishTypeSkipButton];
    }
